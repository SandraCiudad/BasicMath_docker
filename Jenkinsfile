            ...
            steps {
                ...
                script {
                    dir("${env.WORKSPACE}") {
                        ...
                        sh '''cppcheck --enable=all --inconclusive --xml --xml-version=2 `find "/home" -name "CBasicMath.cpp" "TestBasicMath.cpp" | grep -v ".cccc" | grep -v ".svn" | grep -v ".settings" | grep -v ".cproject"` 2> reports/project_cppcheck.xml'''

                        // CCCC Code Analysis
                        sh '''cccc --html_outfile=index.html `find "/home" -name "CBasicMath.cpp" "TestBasicMath.cpp" | grep -v ".svn" | grep -v ".cccc" | grep -v ".settings" | grep -v ".cproject"`; mv .cccc reports/cccc; mv index.html reports/cccc'''
